  ...
  init() {
    ...
    for (let i = 0; i < this.ballAmount; i++) {
      ...
      for (let k = 0; k < this.ballAmountY; k++) {
        ...
        row.push(new Ball((i * this.ballRadius * 2) + this.ballRadius, (k * this.ballRadius * 2), "0x" + rowColors[i], transparency, transparencySpeed));
      }
      this.balls.push(row);
    }
  ...
  draw() {
    for (let i = 0; i < this.balls.length; i++) {
      for (let k = 0; k < this.balls[i].length; k++) {
        ...
        this.graphics.drawCircle(ball.x, ball.y, this.ballRadius - this.padding);
  ...
  update() {
    ...
    for (let i = 0; i < this.balls.length; i++) {
      for (let k = 0; k < this.balls[i].length; k++) {
        ...
        if (this.balls[i][k].transparency >= 1 || this.balls[i][k].transparency <= 0) {
