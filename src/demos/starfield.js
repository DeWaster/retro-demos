...
class Star extends PIXI.Sprite{
  constructor(texture, x, y, transparency, transparencySpeed, speed, direction, maxScale) {
    super(texture);
...
class Starfield {
  ...
  init() {
    ...
    for (let i = 0; i < this.starAmount; i++) {
      ...
      const scale = (Math.random() * (this.stars[this.stars.length - 1].maxScale - 0.05) + 0.05);
  ...
  update() {
    for (let i = 0; i < this.stars.length; i++) {
      ...
      if (this.stars[i].y < 0 || this.stars[i].y > this.height || this.stars[i].x < 0 || this.stars[i].x > this.width) {
  ...
  createStar() {
    ...
    const star = new Star(this.starTexture, this.screenCentre.x,this.screenCentre.y, transparency, transparencySpeed, speed, direction);
