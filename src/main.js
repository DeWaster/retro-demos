...
import skyline from "./assets/vertical/skyline.png";
import skyline2 from "./assets/vertical/skyline2.png";
import moon from "./assets/vertical/moon.png";
import star from "./assets/starfield/star.png";
import nebula from "./assets/starfield/nebula.jpg";
...
const verticalLines = new VerticalLines(
  window.innerWidth,
  window.innerHeight,
  app
);
...
// Load all textures
loader
  .add("skylineTexture", skyline)
  .add("skylineTexture2", skyline2)
  .add("moon", moon)
  .add("starTexture", star)
  .add("nebula", nebula);

loader.load((loader, resources) => {
  // Connect sprites
  verticalLines.setSprite(
    "skylineTexture",
    new PIXI.Texture(resources.skylineTexture.texture)
  );
  verticalLines.setSprite(
    "skylineTexture2",
    new PIXI.Texture(resources.skylineTexture2.texture)
  );
  verticalLines.setSprite("moon", new PIXI.Sprite(resources.moon.texture));
  verticalLines.setSprite(
    "starTexture",
    new PIXI.Sprite(resources.starTexture.texture)
  );
  starfield.setSprite("nebula", new PIXI.Sprite(resources.nebula.texture));
  starfield.setSprite(
    "starTexture",
    new PIXI.Texture(resources.starTexture.texture)
  );
  init();
});
